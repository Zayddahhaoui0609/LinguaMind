...
let startAppBtn, guidePromptModal, guideYesBtn, guideNoBtn,
    userGuideModal, guideContentContainer, guidePrevBtn, guideNextBtn;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    guidePromptModal = new bootstrap.Modal(document.getElementById('guidePromptModal'));
    guideYesBtn = document.getElementById('guideYesBtn');
    guideNoBtn = document.getElementById('guideNoBtn');
    userGuideModal = new bootstrap.Modal(document.getElementById('userGuideModal'));
    guideContentContainer = document.getElementById('guideContent');
    guidePrevBtn = document.getElementById('guidePrev');
    guideNextBtn = document.getElementById('guideNext');
    ...
    async function loadGuideData() {
        ...
        try {
            ...
            // Accept either direct array or {pages:[...]} structure
            if (Array.isArray(json)) {
                guideData = json;
            } else if (Array.isArray(json.pages)) {
                guideData = json.pages;
            } else {
                console.error('Unexpected guide JSON format', json);
                guideData = [];
            }
        } catch (err) {
            console.error('Failed to load guide.json', err);
        }
    }

    function renderGuide() {
        if (!guideContentContainer || !guideData.length) return;
        guideContentContainer.innerHTML = '';
        const pageObj = guideData[currentGuidePage] || {};
        const items = pageObj.Screenshots || pageObj.images || [];
        ...
        guideContentContainer.appendChild(row);
        // Update buttons
        guidePrevBtn.disabled = currentGuidePage === 0;
        guideNextBtn.textContent = currentGuidePage === guideData.length - 1 ? 'Close' : 'Next';
    }

    // Guide button events
    if (guideYesBtn) guideYesBtn.addEventListener('click', async () => {
        sessionStorage.setItem('guideSeen', 'true');
        guidePromptModal.hide();
        await loadGuideData();
        currentGuidePage = 0;
        renderGuide();
        userGuideModal.show();
    ...
    if (guideNoBtn) guideNoBtn.addEventListener('click', () => {
        sessionStorage.setItem('guideSeen', 'true');
    });

    if (startAppBtn) startAppBtn.addEventListener('click', () => {
        guidePromptModal.show();
    });
    ...
    if (guideNextBtn) guideNextBtn.addEventListener('click', () => {
        if (currentGuidePage < guideData.length - 1) {
            currentGuidePage++;
            renderGuide();
        } else {
            userGuideModal.hide();
    ...
    if (!sessionStorage.getItem('guideSeen')) {
        guidePromptModal.show();
    }
